...
async def fetch_attom_comps(subject: dict, radius: int = 2) -> List[dict]:
    ...
    # FINAL FIX: The 'state' variable was missing here
    state = components.get("state")
    postal_code = components.get("postal_code")

    if not all([street, city, state, postal_code]): return []

    url = f"https://api.gateway.attomdata.com/propertyapi/v1.0.0/comparables/detail"
    params = {
        "address1": street,
...
def get_clean_comps(subject: dict, comps: List[dict]) -> Tuple[List[dict], float]:
    ...
    for radius, grade in tiers:
        ...
        for comp in comps:
            prop_details = comp.get("summary") or comp
            
            sold = prop_details.get("lastSoldPrice") or (prop_details.get("sale") or {}).get("saleamt")
