...
async def get_subject_data(address: str) -> Tuple[dict, dict]:
    ...
    # Fetch Zillow details if ZPID available
    if zpid:
        subj_ids["zpid"] = zpid
    ...
    # ATTOM fallback for missing fields
...
async def fetch_attom_fallback(subject: dict, radius: int = 10, count: int = 50) -> List[dict]:
    ...
    if lat is None or lon is None:
        return []
    # Transactions endpoint (may need correction)
...
async def get_comp_summary_by_zpid(
    ...
    """
    Fetch comps and enrich with last sale using a known ZPID and subject info.
    """
    if manual_sqft:
